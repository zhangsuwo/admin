<template>
  <el-dialog v-model="visible" :title="titleMap[mode]" size="70%" destroy-on-close @closed="$emit('closed')">
    <el-form ref="dialogForm" :model="form" :rules="rules" :disabled="mode == 'show'" label-width="9rem" label-position="center">
      ...
      <el-form-item prop="useType" label="使用">
      ...
      <el-form-item prop="ports" label="监听端口">
        <el-input v-model="form.ports" placeholder="ports" clearable></el-input>
      </el-form-item>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        ports: [{ required: true, message: '请输入Docker端口' }],
      },
    }
  },
  mounted() {
    ...
    submit() {
      this.$refs.dialogForm.validate(async (valid) => {
        if (valid) {
          ...
          var res
          if (this.mode === 'add') {
            res = await this.$API.res.server.add.post(this.form)
          } else if (this.mode === 'edit') {
            res = await this.$API.res.server.update.post(this.form)
          }
