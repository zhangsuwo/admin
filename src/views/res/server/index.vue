  ...
  <el-container>
    ...
    <el-main class="nopadding">
      <scTable ref="table" :apiObj="apiObj" :params="query" row-key="id" stripe highlightCurrentRow>
        ...
        <el-table-column label="端口" prop="ports" show-overflow-tooltip> </el-table-column>
        ...
        <el-table-column label="操作" width="250">
          <template #default="scope">
            <el-button-group>
              ...
              <el-button text type="primary" size="small" @click="createDocker(scope.row, scope.$index)">生成Docker</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      apiObj: this.$API.res.server.list,
  ...
  methods: {
    ...
    //生成Docker
    async createDocker(row) {
      var reqData = { id: row.id }
      var res = await this.$API.res.server.createDocker.post(reqData)
      if (res.code === 0) {
        this.$refs.table.refresh()
        this.$message.success('创建Docker成功')
      } else {
        this.$alert(res.message, '提示', { type: 'error' })
      }
    },
